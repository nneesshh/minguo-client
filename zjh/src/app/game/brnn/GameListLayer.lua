...
function GameListLayer:showPlayerList(list)
    ...
    for i = 1,#list do
        ...
        if list[i].seqid <= 5 then            
            f_rank:setVisible(false)
            i_rank:setVisible(true)
            i_rank:ignoreContentAdaptWithSize(true)
            i_rank:loadTexture(string.format("game/brnn/image/img_rank_%d.png", list[i].seqid), ccui.TextureResType.plistType)
        else
...
function GameListLayer:showSelfList(list)    
    ...
    if list.seqid <= 5 then            
        f_rank:setVisible(false)
        i_rank:setVisible(true)
        i_rank:ignoreContentAdaptWithSize(true)
        i_rank:loadTexture(string.format("game/brnn/image/img_rank_%d.png", list.seqid), ccui.TextureResType.plistType)
    else
        i_rank:setVisible(false)
        f_rank:setVisible(true)
